...
- 平坦化
    - **flat** : 配列の中の配列を展開し、指定値以上の深さが無くなった配列を返します。([MDN](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Array/flat))
    - **flatMap** : `map()`と同じ引数で全ての要素に関数を実行した後、`flat()`を行います。([MDN](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Array/flatMap))
- 文字列化
