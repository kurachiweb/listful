...
const PostPage = ({ post, categories }) => (
  <Wrapper url="/" title={`${post.title} | ${config.title}`} description={post.excerpt} categories={categories} imageUrl={config.url + post.coverImage} imageAlt={post.coverImageAlt}>
    <BlogPost post={post} />
  </Wrapper>
);

export async function getStaticProps({ params }) {
  const thisPost = getPostBySlug(params.slug, ['title', 'excerpt', 'date', 'slug', 'author', 'category', 'content', 'coverImage', 'coverImageAlt', 'coverImageHeight', 'coverImageWidth']);
